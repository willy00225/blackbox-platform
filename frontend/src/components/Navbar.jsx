    ...
    <>
      ...
      <header className="fixed top-0 w-full z-50 bg-carbon/80 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
          ...
          <button onClick={() => setIsDrawerOpen(true)} className="lg:hidden text-white p-2">
            <Menu className="w-6 h-6" />
          </button>

          {/* Logo (cliquable pour retour accueil) */}
          <Link to="/" onClick={onLogoClick} className="flex items-center cursor-pointer">
            <img src="/src/assets/logo.png" alt="Black Box" className="h-8 w-auto object-contain" />
          </Link>

          {/* Navbar Desktop */}
          <nav className="hidden lg:flex gap-6 text-sm font-medium text-gray-300">
            {navItems.map(item => (
              <Link key={item.path} to={item.path} className={`hover:text-gold transition ${isActive(item.path) ? 'text-gold' : ''}`}>
                {item.label}
              </Link>
            ))}
          </nav>
          ...
          <div className="lg:hidden flex items-center gap-3">
            ...
            {/* Sur mobile, on peut aussi afficher l'avatar si on veut */}
            <div 
              onClick={onProfileClick}
      ...
      {isDrawerOpen && (
        <div className="fixed inset-0 z-[100] lg:hidden">
          ...
          {/* Panneau Latéral */}
          <div className="absolute left-0 top-0 bottom-0 w-72 bg-deepblack border-r border-gray-800 p-6 flex flex-col">
            <div className="flex justify-between items-center mb-8">
              <img src="/src/assets/logo.png" alt="Black Box" className="h-8 w-auto" />
            ...
            <div className="flex-1 space-y-2">
              {navItems.map(item => (
                <Link
                  ...
                  className={`flex items-center gap-3 p-3 rounded-lg transition ${isActive(item.path) ? 'bg-gold/10 text-gold' : 'text-gray-300 hover:bg-gray-800'}`}
                >
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 p-3 rounded-lg transition text-left ${location.pathname === '/profile' ? 'bg-gold/10 text-gold' : 'text-gray-300 hover:bg-gray-800'}`}
              >
            ...
            {/* Déconnexion */}
            <button onClick={() => { setIsDrawerOpen(false); onLogout(); }} className="mt-auto flex items-center gap-3 p-3 text-crimson hover:bg-crimson/10 rounded-lg transition">
